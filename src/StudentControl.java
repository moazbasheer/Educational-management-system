...
public class StudentControl {
	...
	public void executeStudentMenuChoice(int x){
		if(x==1) registerInCourse();
		else if(x==2) ViewCourses();
		else if(x==3) ListCourses();
		else{
			currentStudentnum=-1;
			currentCoursenum=-1;
			logout=1;
	...
	public void ViewCourses(){
		...
		int choice=getChoice(1,Database.students.get(currentStudentnum).registered_course.size());
		currentCoursenum=choice-1;
		System.out.print("Course name: "+Database.courses.get(currentCoursenum).name+"\n");
		System.out.print("Course ID: "+Database.courses.get(currentCoursenum).ID+"\n");
		System.out.print("Doctor name: "+Database.courses.get(currentCoursenum).docName+"\n\n");
		...
		for(int i=0;i<Database.courses.get(currentCoursenum).stud.size();i++){
			if(Database.courses.get(currentCoursenum).stud.get(i).name.equals(Database.students.get(currentStudentnum).name)){
				stnum=i;
				break;
			}
		}
		...
		for(int i=0;i<sz;i++){
			printAssignment(Database.courses.get(currentCoursenum).stud.get(stnum).assignment.get(i),i+1);
		}
		
		System.out.println();
		
		System.out.print("1-Unregister course\n2-Submit Solution\n3-Back\n");
	...
	public void printAssignment(Assignment x,int num){
		...
		if(x.grade!=-1) System.out.print(x.grade);
	...
	public void SubmitSolution(int stnum){
		...
		for(int i=0;i<sz;i++){
			printAssignment(Database.courses.get(currentCoursenum).assignment.get(i),i+1);
		}
		...
		}while(Database.courses.get(currentCoursenum).stud.get(stnum).assignment.get(choice-1).submitted==true);
		System.out.print("The question is :\n");
		System.out.println(Database.courses.get(currentCoursenum).stud.get(stnum).assignment.get(choice-1).question);
		System.out.print("Write your answer\n");
		String s=scan.nextLine();
		Database.courses.get(currentCoursenum).stud.get(stnum).assignment.get(choice-1).answer=scan.nextLine();
		Database.courses.get(currentCoursenum).stud.get(stnum).assignment.get(choice-1).submitted=true;
	...
	public void UnregisterCourse(){
		...
		for(int i=0;i<sz;i++){
			if(Database.courses.get(currentCoursenum).stud.get(i).equals(Database.students.get(currentStudentnum))){
		...
		for(int i=0;i<sz;i++){
			if(Database.students.get(currentStudentnum).registered_course.get(i).equals(Database.courses.get(currentCoursenum))){
				Database.students.get(currentStudentnum).registered_course.remove(i);
				break;
			}
		}
	...
	public void ListCourses(){
		int sz=Database.students.get(currentStudentnum).registered_course.size();
		System.out.println("Your have "+sz+" courses :\n");
		
		for(int i=0;i<sz;i++){
			System.out.println((i+1)+"-"+Database.students.get(currentStudentnum).registered_course.get(i).name);
		}
	...
	public boolean isRegistered(Course cr){
		for(int i=0;i<Database.students.get(currentStudentnum).registered_course.size();i++){
			if(Database.students.get(currentStudentnum).registered_course.get(i).equals(cr)){
				return true;
			}
		}
		return false;
	...
	public void registerInCourse(){
		...
		Database.students.get(currentStudentnum).registered_course.add(reg.get(choice-1));
		int x=-1;
		...
		Database.courses.get(x).stud.add(Database.students.get(currentStudentnum));
		int n=Database.courses.get(x).stud.size()-1;
		//Add assignments to student
		for(int i=0;i<Database.courses.get(x).assignment.size();i++){
			Database.courses.get(x).stud.get(n).assignment.add(Database.courses.get(choice-1).assignment.get(i));
		}
		System.out.print("Your registeration completed\n");
		}
		else System.out.println("Sorry there is no unregistered courses\n");
	}
	
	public void executeSignInUp(int choice){
		if(choice==1){
			currentStudentnum=SignIn();  //SignIn() gets the object of the student that logged in
		}
		else if(choice==2){
			SignUp();
			signup=1;
	...
	public int getStudentMenuChoice(){
		int choice;
		System.out.print("Please make a choice:\n");
		System.out.print("1-Register in course\n2-View a course\n3-List Courses\n4-Log out\n");
		do {
			System.out.print("Please Enter valid choice:\n");
			choice=scan.nextInt();
	...
	void SignUp(){
		...
		while(!Database.isValidStudentUsername(name)){
			System.out.println("There is another user has same name:\nTry again\n");
			System.out.print("Please Enter your username\n");
			name=scan.nextLine();
	...
	int SignIn(){
		...
		while(Database.getStudent(name,pass)==-1){ 
			System.out.print("The username or the password is wrong\nPlease try again\n");
			System.out.print("Please Enter your username\n");
			name=scan.nextLine();
