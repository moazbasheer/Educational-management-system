...
public class DoctorControl {
	...
	public void ListCourses(){
		...
		for(int i=0;i<x;i++){
			System.out.println((i+1)+"-"+Database.doctors.get(currentDoctornum).course.get(i).name);
		}
	}
	...
	public void CreateCourse(){
		...
		Course r=new Course(nam,id,Database.doctors.get(currentDoctornum).name);
		Database.courses.add(new Course(nam,id,Database.doctors.get(currentDoctornum).name));
	...
	public void ViewCourse(){
		...
		System.out.print("1-Create Assignment\n2-View Student\n3-Show grades report\n4-Back\n");
	...
	public void ShowGradesReport(){
		for(int i=0;i<Database.courses.get(currentCoursenum).stud.size();i++)
		{
			...
			for(int j=0;j<Database.courses.get(currentCoursenum).stud.get(i).assignment.size();j++)
			{
				
				totalgrades+=Database.courses.get(currentCoursenum).stud.get(i).assignment.get(j).grade;
				ttotalgrades+=Database.courses.get(currentCoursenum).stud.get(i).assignment.get(j).fullgrade;
				
				if(Database.courses.get(currentCoursenum).stud.get(i).assignment.get(j).submitted==true){
					numRegistered++;
				}
				
			}
			System.out.println("Student's name is: "+Database.courses.get(currentCoursenum).stud.get(i).name);
			System.out.println("number of submitted is: "+numRegistered);
			System.out.println("Total grades: "+totalgrades+"/"+ttotalgrades);
	...
	public void CreateAssignment(){
		...
		Database.courses.get(currentCoursenum).assignment.add(new Assignment(q,a));
		int x=Database.courses.get(currentCoursenum).stud.size();
		for(int i=0;i<x;i++){
			Database.courses.get(currentCoursenum).stud.get(i).assignment.add(new Assignment(q,a));
		}
	}
	...
	public int ViewStudent(){
		...
		System.out.print("If you want to get back enter -1 otherwise any number:\n");
		int ch=scan.nextInt();
		if(ch==-1)
			return -1;
		...
		int choice=getChoice(1,x);
		Student st=Database.courses.get(currentCoursenum).stud.get(choice-1);
		...
		if(choice2==1){
			...
			System.out.println(Database.courses.get(currentCoursenum).stud.get(choice-1).assignment.get(r-1).fullgrade);
			int gr=scan.nextInt();	
			Database.courses.get(currentCoursenum).stud.get(choice-1).assignment.get(r-1).grade=gr;
		}
		else if(choice2==2){
			System.out.print("Please enter a comment:\n");
			String s=scan.nextLine();
			Database.courses.get(currentCoursenum).stud.get(choice-1).assignment.get(r).comment=s;
		}
		return 0;
	}
